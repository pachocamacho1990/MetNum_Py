...
def reshape_data(x,y,df):
    # this function reshapes data from pandas to mincua.
...
def reshape_scikit(xvars,y,df):
    # this function reshapes data from pandas 
    # to scikit-learn ideal format for regresion
    # df     -> dataframe from pandas
    # xvars(str) -> column names acting as x variables
    #   ex: ['var_1','var_2',...,'var_n']
    # y(str) -> column name acting as y variable
    X_data = []
    Y_data = []
    for i in range(len(df)):
        temp = []
        for p in xvars:
            temp.append(df.iloc[i][p])
        X_data.append(temp)
        Y_data.append(df.iloc[i][y])
